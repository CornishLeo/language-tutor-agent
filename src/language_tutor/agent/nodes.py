...
def call_japanese_tutor(state: AgentState, runtime: Runtime[AgentContext] = None):
    ...
    model_name, prompt, temperature = get_node_config(
        "japanese_tutor", version=config_version
    )
    ...
    return {
        ...
        "chat_history": [HumanMessage(state["user_input"]), AIMessage(response.text)],
    }
...
def call_grammar_critic(state: AgentState, runtime: Runtime[AgentContext] = None):
    ...
    llm = get_structured_llm(
        model_name=model_name, schema=GrammarCriticSchema, temperature=temperature
    )

    review: GrammarCriticSchema = llm.invoke(
        [SystemMessage(prompt), HumanMessage(state["user_input"])]
    )

    # Go to the "japanese_tutor" node, but first update feedback if given
    if review.is_correct:
        # Path A: User's input was correct, no feedback given.
        return Command(update={"feedback_notes": ""}, goto="japanese_tutor")
    else:
